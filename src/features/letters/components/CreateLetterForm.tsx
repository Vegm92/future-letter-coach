...
import { useVoiceMemoRecorder } from "@/features/voice-memo/hooks/useVoiceMemoRecorder";
        ...
        if (milestonesToInsert.length > 0) {
          ...
          if (milestoneError) {
            ...
            toast({
              ...
              description:
                "You can add milestones later from the letter details.",
              variant: "destructive",
            });
          }
        }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {enhancement.hasEnhancementData && enhancement.data && (
                <Collapsible
                  open={enhancement.isExpanded}
                  onOpenChange={enhancement.setIsExpanded}
                >
                    ...
                    <div className="space-y-4 rounded-lg bg-muted/50 p-4">
                      <div className="flex justify-between items-center">
                        ...
                        <Button
                          ...
                          disabled={
                            enhancement.appliedFields.size === 3 &&
                      ...
                      {enhancement.data.suggestedMilestones &&
                        enhancement.data.suggestedMilestones.length > 0 && (
                          <div className="space-y-2">
                            <div className="flex justify-between items-center">
                              <Label className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                                Suggested Milestones (
                                {enhancement.data.suggestedMilestones.length})
                                {enhancement.milestonesApplied && (
                                  <Check className="h-3 w-3 text-green-500" />
                                )}
                              </Label>
                            ...
                            <div className="space-y-2 max-h-32 overflow-y-auto">
                              {enhancement.data.suggestedMilestones.map(
                                (milestone, index) => (
                                  <div
                                    key={index}
              ...
              {showMilestones && (
                <div className="space-y-4 p-4 bg-muted/30 rounded-lg border">
                  <p className="text-sm text-muted-foreground">
                    Break down your goal into smaller, measurable milestones to
                    track your progress.
                  </p>
            ...
            <div className="flex flex-col gap-2 p-4 bg-muted/30 rounded-lg border">
              <div className="flex items-center justify-between">
                <span className="font-medium">Voice Memo (Optional)</span>
              </div>
              <div className="flex gap-2 flex-wrap">
                {/* Record button (only show if not recording) */}
                {!isRecording && (
                  <Button
                    type="button"
                    variant="outline"
                ...
                {/* Pause/Resume/Stop buttons (show if recording) */}
                {isRecording && (
                  <>
                    {/* Pause or Resume */}
                    {!isPaused ? (
                    ...
                    {/* Stop always visible when recording */}
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                ...
                {/* Reset/Remove button (show if audio exists and not recording) */}
              ...
              {/* Audio preview (show if audio exists and not recording) */}
              {audioUrl && !isRecording && (
              ...
              {/* Only show one set of voice memo controls below */}
            </div>
